...
/** Minimal rational number type for exact fractional time positions. Used by
  ...
  * imprecision. Denominators in practice are products of small primes (2, 3, 5,
  * 7) from Pulse subdivision factors.
  */
case class Rational private (num: Long, den: Long) extends Ordered[Rational]:
  ...
  def *(that: Rational): Rational = Rational(num * that.num, den * that.den)

  def /(that: Rational): Rational =
    require(that.num != 0, "Division by zero")
    Rational(num * that.den, den * that.num)

  def compare(that: Rational): Int = (num * that.den).compareTo(that.num * den)

  override def toString: String = if den == 1 then s"$num" else s"$num/$den"
