...
class PartWritingSuite extends munit.FunSuite:
  ...
  test("parallel fifths detected"):
    ...
    val result  = PartWriting.check(List(soprano, bass), NoteType.C, Scale.Major)
  ...
  test("fromVoices — I → ii with parallel fifths reports both analysis and errors"):
    ...
    assert(result.partWriting.isInvalid)
    val errors = result.partWriting.fold(_.toList, _ => Nil)
  ...
  test("fromVoices — correct I → V returns valid part-writing with analysis"):
    val soprano = voice(E(4), D(4))
    val alto    = voice(C(4), B(3))
    val tenor   = voice(G(3), G(3))
    val bass    = voice(C(3), G(2))
    ...
    assert(result.partWriting.isValid)
